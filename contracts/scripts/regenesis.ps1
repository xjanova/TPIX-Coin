...
if (Test-Path "$walletDir\wallets.json") {
    Write-Host "❌ wallet-output\ already has a wallet generated." -ForegroundColor Red
...
if ($pwd1 -ne $pwd2) { Write-Host "❌ Passwords don't match." -ForegroundColor Red; exit 1 }
if ($pwd1.Length -lt 12) { Write-Host "❌ Too short (12+)." -ForegroundColor Red; exit 1 }
...
Write-Host ""
Write-Host "⚠️  After backup, BOTH plaintext files MUST be deleted from disk." -ForegroundColor Yellow
Write-Host ""
...
Write-Host "  ✓ Deleted mnemonic.txt + privatekeys.txt"
Write-Host "  ✓ Encrypted keystores preserved: master-wallet.keystores.json"
...
Write-Host " ✅ Local regenesis prep complete." -ForegroundColor Green
Write-Host "════════════════════════════════════════════════════" -ForegroundColor Green
Write-Host ""
